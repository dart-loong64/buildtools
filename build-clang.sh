...
fuchsia_root="$build_root/fuchsia"
...
echo "==> Patching Fuchsia-stage2.cmake for ${llvm_backend}"
if ! grep -Eq "(^|[; ])${llvm_backend}([; )]|$)" "$stage2_cache"; then
  sed -i -E "s/^(set\\(LLVM_TARGETS_TO_BUILD [^)]*) CACHE/\\1;${llvm_backend} CACHE/" \
    "$stage2_cache"
fi
if ! grep -q "$target_triple" "$stage2_cache"; then
  sed -i -E "s/^(foreach\\(target [^)]*)\\)/\\1;${target_triple})/" \
    "$stage2_cache"
fi
